...
$sel_alb="SELECT * FROM albaranes_elec WHERE codalbaran='$codalbaran'";
...
$sel_cliente="SELECT name_item_tramos FROM item_tramos item JOIN albaranes_elec alba ON alba.id_item_tramos=item.idItem_tramos WHERE alba.codalbaran='$codalbaran'";
$rs_cliente=mysql_query($sel_cliente);
$nombre=mysql_result($rs_cliente,0,"name_item_tramos");

$fechahoy=date("Y-m-d");
$sel_albaran="INSERT INTO albaranestmp_elec (codalbaran,fecha) VALUE ('','$fechahoy')";
...
while ($contador < mysql_num_rows($rs_lineas)) {
	...
	$sel_tmp="INSERT INTO albalineatmp_elec (codalbaran,numlinea,codfamilia,codigo,cantidad,precio,importe,observacion) VALUES ('$codalbarantmp','','$codfamilia','$codigo','$cantidad','$precio','$importe','$observacion')";
