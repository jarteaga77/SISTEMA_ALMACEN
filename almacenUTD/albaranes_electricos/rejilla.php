...
$where="1=1";
if ($codcliente <> "") { $where.=" AND albaranes_elec.id_Item_tramos='$codcliente'"; }
if ($nombre <> "") { $where.=" AND item_tramos.name_item_tramos like '%".$nombre."%'"; }
if ($numalbaran <> "") { $where.=" AND albaranes_elec.codalbaran='$numalbaran'"; }
if ($estado > "0") { $where.=" AND albaranes_elec.estado='$estado'"; }
if (($fechainicio<>"") and ($fechafin<>"")) {
	$where.=" AND albaranes_elec.fecha between '".$fechainicio."' AND '".$fechafin."'";
} else {
	if ($fechainicio<>"") {
		$where.=" and albaranes_elec.fecha>='".$fechainicio."'";
	} else {
		if ($fechafin<>"") {
			$where.=" and albaranes_elec.fecha<='".$fechafin."'";
		}
	}
}

$where.=" ORDER BY albaranes_elec.codalbaran DESC";
$query_busqueda="SELECT count(*) as filas 
FROM albaranes_elec,item_tramos, tramos
WHERE albaranes_elec.id_item_tramos=item_tramos.idItem_tramos
AND item_tramos.Tramos_idTramos=tramos.idTramos AND
albaranes_elec.borrado=0 AND  ".$where;
		...
		<div id="pagina">
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<? echo $filas?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<? $sel_resultado="SELECT albaranes_elec.codalbaran,albaranes_elec.activo,
CONCAT(centrocostos.nombre_ccostos, '-', tramos.name_tramo, '-', item_tramos.name_item_tramos)AS centro, 
albaranes_elec.fecha,albaranes_elec.totalalbaran
FROM albaranes_elec, centrocostos, tramos, item_tramos 
WHERE item_tramos.idItem_tramos=albaranes_elec.id_item_tramos
AND tramos.idTramos= item_tramos.Tramos_idTramos 
AND centrocostos.id_ccostos=tramos.centrocostos_id_ccostos AND albaranes_elec.borrado=0 AND " .$where;
						   $sel_resultado=$sel_resultado." limit ".$iniciopagina.",10";
						   $res_resultado=mysql_query($sel_resultado);
						   $contador=0;
						   //$marcaestado=0;					   
						   while ($contador < mysql_num_rows($res_resultado)) { 
						//   		$marcaestado=mysql_result($res_resultado,$contador,"albaranes_elec.estado");
				
				$activo=mysql_result($res_resultado,$contador,"albaranes_elec.activo");				
						...
						<tr class="<?php echo $fondolinea?>">
							...
							<td width="1%"><div align="center"><? echo mysql_result($res_resultado,$contador,"albaranes_elec.codalbaran")?></div></td>
							<td width="27%"><div align="center"><? echo mysql_result($res_resultado,$contador,"centro")?></div></td>
							<td width="5%"><div align="center"><? echo number_format(mysql_result($res_resultado,$contador,"albaranes_elec.totalalbaran"),2,".",",")?></div></td>							
							<td class="aDerecha" width="10%"><div align="center"><? echo mysql_result($res_resultado,$contador,"albaranes_elec.fecha")?></div></td>
							<td width="5%"><div align="center"><a href="#"><img src="../img/modificar.png" width="16" height="16" border="0" onClick="modificar_albaran(<?php echo mysql_result($res_resultado,$contador,"albaranes_elec.codalbaran")?>,<? echo $level?>,<? echo $activo ?>)" title="Modificar"></a></div></td>
							<td width="5%"><div align="center"><a href="#"><img src="../img/ver.png" width="16" height="16" border="0" onClick="ver_albaran(<?php echo mysql_result($res_resultado,$contador,"albaranes_elec.codalbaran")?>)" title="Visualizar"></a></div></td>
						</tr>
						<? $contador++;
							}
						?>			
