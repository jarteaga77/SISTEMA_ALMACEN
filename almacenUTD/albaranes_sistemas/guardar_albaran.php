...
if($accion == "alta") {
	$query_operacion="INSERT INTO albaranes_sys (codalbaran, codfactura, fecha, iva, id_item_tramos, estado, borrado, id_usuario,transporta_reviza,recibe,observacion,requisicion,activo) VALUES ('', '0', '$fecha', '$iva', '$codcliente', '1', '0', $idusu, '$reviza', '$recibe','$obse', '$req', '$activo' )";					
	$rs_operacion=mysql_query($query_operacion);
	$codalbaran=mysql_insert_id();
	if ($rs_operacion) { $mensaje="El albar&aacute;n ha sido dado de alta correctamente"; }
	$query_tmp="SELECT * FROM albalineatmp_sys WHERE codalbaran='$codalbarantmp' ORDER BY numlinea ASC";
	...
	while ($contador < mysql_num_rows($rs_tmp)) {
		...
		$sel_insertar="INSERT INTO albalinea_sys (codalbaran,numlinea,codfamilia,codigo,cantidad,precio,importe,observacion) VALUES 
		...
		if ((mysql_result($rs_minimos,0,"stock") < mysql_result($rs_minimos,0,"stock_minimo")) or (mysql_result($rs_minimos,0,"stock") <= 0))
			...
			$sel_control_articulos="INSERT INTO control_articulos (fecha,codarticulo,stock_antiguo,cantidad_retirada,stock_nuevo,modulo_mod,idusuario)VALUES('$fecha1','$codigo','$stock_antiguo','$cantidad','$stock_nuevo','REMS','$idusu')";
	...
	$cabecera2="INSERTAR Remisión SISTEMAS ";
}

if ($accion=="modificar") {
	$codalbaran=$_POST["codalbaran"];
	$act_albaran="UPDATE albaranes_sys SET fecha_mod='$fecha1', iva='$iva',id_usuario_mod='$idusu', requisicion='$req', activo='$activo' WHERE codalbaran='$codalbaran'";
	$rs_albaran=mysql_query($act_albaran);
	$sel_lineas = "SELECT codigo,codfamilia,cantidad FROM albalinea_sys WHERE codalbaran='$codalbaran' order by numlinea";
	...
	$sel_borrar = "DELETE FROM albalinea_sys WHERE codalbaran='$codalbaran'";
	$rs_borrar = mysql_query($sel_borrar);
	$sel_lineastmp = "SELECT * FROM albalineatmp_sys WHERE codalbaran='$codalbarantmp' ORDER BY numlinea";
	...
	while ($contador < mysql_num_rows($rs_lineastmp)) {
		...
		$sel_insert = "INSERT INTO albalinea_sys (codalbaran,numlinea,codigo,codfamilia,cantidad,precio,importe,observacion) 
	...
	$cabecera2="MODIFICAR Remisi&Aacute;n  Sistemas";
}
...
<html>
	<head>
		...
		function imprimir(codalbaran) {
			window.open("../fpdf/imprimir_albaran_sys.php?codalbaran="+codalbaran);
		}
			...
			<div id="zonaContenido">
				...
				<div id="frmBusqueda">
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0>
						...
						<? } 
						 $sel_cliente="SELECT name_item_tramos, idItem_tramos FROM item_tramos item JOIN albaranes_sys alba ON alba.id_item_tramos=item.idItem_tramos WHERE alba.codalbaran='$codalbaran'"; 
					...
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0 ID="Table1">
					  <? $sel_lineas="SELECT albalinea_sys.*,articulos.*,familias.nombre as nombrefamilia FROM albalinea_sys,articulos,familias WHERE albalinea_sys.codalbaran='$codalbaran' AND albalinea_sys.codigo=articulos.codarticulo AND albalinea_sys.codfamilia=articulos.codfamilia AND articulos.codfamilia=familias.codfamilia ORDER BY albalinea_sys.numlinea ASC";
$rs_lineas=mysql_query($sel_lineas);
