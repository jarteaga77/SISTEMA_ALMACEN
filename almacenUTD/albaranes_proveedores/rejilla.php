...
<html>
	<head>
		...
		function modificar_albaran(codalbaran,level,activo,codproveedor) {
			...
			{
				alert("La orden de Entrada no esta activa. Comuniquese con el Administrador");
		...
		<div id="pagina">
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<? echo $filas?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<? $sel_resultado="SELECT codalbaran,proveedores.nombre as nombre,albaranesp.fecha as fecha,activo,proveedores.codproveedor,totalalbaran,estado FROM albaranesp,proveedores WHERE albaranesp.codproveedor=proveedores.codproveedor AND ".$where;
						 ...
						 $activo=mysql_result($res_resultado,$contador,"activo");  
						   
						   if (mysql_result($res_resultado,$contador,"estado")==1) { $estado="Sin facturar"; } else { $estado="Facturado"; }
								if ($contador % 2) { $fondolinea="itemParTabla"; } else { $fondolinea="itemImparTabla"; }?>
						<tr class="<?php echo $fondolinea?>">
							<td class="aCentro" width="12%"><div align="center"><? echo $contador+1;?></td>
							<td width="8%"><div align="center"><? echo mysql_result($res_resultado,$contador,"codalbaran")?></div></td>
							<td width="25%"><div align="center"><? echo mysql_result($res_resultado,$contador,"nombre")?></div></td>							
							<td width="8%"><div align="center"><? echo number_format(mysql_result($res_resultado,$contador,"totalalbaran"),2,".",",")?></div></td>
							<td class="aDerecha" width="8%"><div align="center"><? echo mysql_result($res_resultado,$contador,"fecha")?></div></td>
							<td width="6%"><div align="center"><a href="#"><img src="../img/modificar.png" width="16" height="16" border="0" onClick="modificar_albaran('<?php echo mysql_result($res_resultado,$contador,"codalbaran")?>',<? echo $level ?>,<? echo $activo ?>,<?php echo mysql_result($res_resultado,$contador,"codproveedor")?>,<? echo $marcaestado?>)" title="Modificar"></a></div></td>
