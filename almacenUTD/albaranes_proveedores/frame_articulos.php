...
<?
	...
	if ($todos==1) {
		$consulta="SELECT articulos.*,familias.nombre as nombrefamilia,ubicaciones.nombre FROM articulos,familias,ubicaciones WHERE ".$where." AND articulos.codfamilia=familias.codfamilia AND articulos.codubicacion=ubicaciones.codubicacion AND articulos.borrado=0 ORDER BY articulos.codfamilia ASC,articulos.descripcion ASC";
	}
	if ($todos==0) {
		$consulta="SELECT artpro.precio as pcosto,articulos.*,familias.nombre as nombrefamilia,ubicaciones.nombre FROM artpro,articulos,familias,ubicaciones
			WHERE ".$where." AND artpro.codarticulo=articulos.codarticulo AND artpro.codfamilia=articulos.codfamilia AND artpro.codproveedor='".$codproveedor."' AND articulos.codfamilia=familias.codfamilia AND articulos.codubicacion=ubicaciones.codubicacion AND articulos.borrado=0 ORDER BY articulos.codfamilia ASC,articulos.descripcion ASC";			
	}
...
<? if ($nrs>0) { ?>
		<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0>
		  <tr>
			...
			<td width="10%"><div align="center"><b>Precio</b></div></td>
             <td width="20%"><div align="center"><b>Ubicaci&oacute;n</b></div></td>
		...
		<?php
			for ($i = 0; $i < mysql_num_rows($rs_tabla); $i++) {
				...
				$ubicacion=mysql_result($rs_tabla,$i,"nombre");
				$stock=mysql_result($rs_tabla,$i,"stock");
        ...
        <div align="left"><?php echo utf8_encode($descripcion);?></div></td>
                    ...
                    <td><div align="center"><?php echo $ubicacion;?></div></td>
