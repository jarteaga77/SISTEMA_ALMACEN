...
if($accion == "alta") {
	$query_operacion="INSERT INTO albaranes (codalbaran, codfactura, fecha, iva, id_ccostos, estado, borrado, id_usuario,transporta_reviza,recibe,observacion,activo) VALUES ('', '0', '$fecha', '$iva', '$codcliente', '1', '0', $idusu, '$reviza', '$recibe','$obse', '$activo' )";					
	...
	while ($contador < mysql_num_rows($rs_tmp)) {
		...
		$sel_insertar="INSERT INTO albalinea (codalbaran,numlinea,codfamilia,codigo,cantidad,precio,importe,observacion) VALUES 
		...
		$sel_minimos = "SELECT stock,stock_minimo,descripcion,fecha_venci,aviso_venci FROM articulos where codarticulo='$codigo' AND codfamilia='$codfamilia'";
		...
		if ((mysql_result($rs_minimos,0,"stock") < mysql_result($rs_minimos,0,"stock_minimo")) or (mysql_result($rs_minimos,0,"stock") <= 0))
	   		{ 
				...
				$minimo=1;
					...
					$fecha_venci=mysql_result($rs_minimos,0,"fecha_venci");
					
				$aviso_ven=mysql_result($rs_minimos,0,"aviso_venci");
					
					
					if($aviso_ven == 1)
					{
					
					if($fecha1 > $fecha_venci)
					{
						$mensaje_venci=$mensaje_minimo . " " . mysql_result($rs_minimos,0,"descripcion")."<br>";				$alerta=1;
					}
					
					}
					
					$sel_control_articulos="INSERT INTO control_articulos (fecha,codarticulo,stock_antiguo,cantidad_retirada,stock_nuevo,modulo_mod,idusuario)VALUES('$fecha1','$codigo','$stock_antiguo','$cantidad','$stock_nuevo','RMG','$idusu')";
...
if ($accion=="modificar") {
	...
	//echo $codalbaran;
	$act_albaran="UPDATE albaranes SET id_ccostos='$codcliente', iva='$iva', fecha_mod='$fecha1', id_usuario_mod='$idusu', activo='$activo' WHERE codalbaran='$codalbaran'";
...
//	echo $act_albaran;
	...
	while ($contador < mysql_num_rows($rs_lineastmp)) {
		...
		$sel_insert = "INSERT INTO albalinea (codalbaran,numlinea,codigo,codfamilia,cantidad,precio,importe,observacion) 
		...
		$sel_bajominimo ="SELECT stock,stock_minimo,descripcion,aviso_venci,fecha_venci FROM articulos WHERE codarticulo='$codigo' AND codfamilia='$codfamilia'";
		...
		if (($stock < $stock_minimo) or ($stock <= 0))
		   ...
		   $fecha_venci=mysql_result($rs_bajominimo,0,"fecha_venci");
					
				$aviso_ven=mysql_result($rs_bajominimo,0,"aviso_venci");
					...
					if($fecha1 > $fecha_venci)
					{
						$mensaje_venci=$mensaje_minimo . " " . mysql_result($rs_bajominimo,0,"descripcion")."<br>";				$alerta=1;
					}
			...
			<div id="zonaContenido">
				...
				<div id="frmBusqueda">
					<table class="fuente8" width="98%" cellspacing=0 cellpadding=3 border=0>
                        ...
                        <tr>
							<td width="15%"></td>
							<td width="85%" colspan="2" class="mensajeminimo">El siguiente articulo esta vencido<br><?php echo $mensaje_venci;?></td>
