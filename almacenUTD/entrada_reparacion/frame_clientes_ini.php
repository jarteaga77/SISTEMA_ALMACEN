...
$iditem=$_POST["cmbfamilia"];
$consulta="SELECT 	idItem_tramos, name_item_tramos FROM item_tramos WHERE Tramos_idTramos = '$iditem'";
...
<? if ($nrs>0) { ?>
		...
		<?php
			for ($i = 0; $i < mysql_num_rows($rs_tabla); $i++) {
				$codcliente=mysql_result($rs_tabla,$i,"idItem_tramos");
				$nombre=mysql_result($rs_tabla,$i,"name_item_tramos");
