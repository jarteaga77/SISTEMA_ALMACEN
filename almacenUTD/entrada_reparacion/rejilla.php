...
if ($numalbaran <> "") { $where.=" AND entrada_reparacion.id='$numalbaran'"; }
//if ($estado > "0") { $where.=" AND albaranes_sys.estado='$estado'"; }
if (($fechainicio<>"") and ($fechafin<>"")) {
	$where.=" AND entrada_reparacion.fecha_entrada between '".$fechainicio."' AND '".$fechafin."'";
} else {
	if ($fechainicio<>"") {
		$where.=" AND entrada_reparacion.fecha_entrada>='".$fechainicio."'";
	} else {
		if ($fechafin<>"") {
			$where.=" AND entrada_reparacion.fecha_entrada<='".$fechafin."'";
		}
	}
}

$where.=" ORDER BY entrada_reparacion.id DESC";
$query_busqueda="SELECT count(*) as filas 
FROM entrada_reparacion,item_tramos, tramos WHERE entrada_reparacion.id_item_tramos=item_tramos.idItem_tramos
AND item_tramos.Tramos_idTramos=tramos.idTramos AND ".$where;
		...
		<div id="pagina">
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<? echo $filas?>">
				...
				if ($iniciopagina>$filas) { $iniciopagina=0; }
					if ($filas > 0) { ?>
						<? $sel_resultado="SELECT entrada_reparacion.id,
CONCAT(tramos.name_tramo, '-', item_tramos.name_item_tramos)AS centro, 
entrada_reparacion.fecha_entrada, entrada_reparacion.activo
FROM entrada_reparacion,tramos,item_tramos 
WHERE item_tramos.idItem_tramos=entrada_reparacion.id_item_tramos
...
AND " .$where;
						  ...
						  $activo=mysql_result($res_resultado,$contador,"entrada_reparacion.activo");	
						...
						<tr class="<?php echo $fondolinea?>">
							...
							<td width="1%"><div align="center"><? echo mysql_result($res_resultado,$contador,"entrada_reparacion.id")?></div></td>
							<td width="27%"><div align="center"><? echo mysql_result($res_resultado,$contador,"centro")?></div></td>
							<td class="aDerecha" width="10%"><div align="center"><? echo mysql_result($res_resultado,$contador,"entrada_reparacion.fecha_entrada")?></div></td>
							<td width="5%"><div align="center"><a href="#"><img src="../img/modificar.png" width="16" height="16" border="0" onClick="modificar_albaran(<?php echo mysql_result($res_resultado,$contador,"entrada_reparacion.id")?>,<? echo $level?>,<? echo $activo?> )" title="Modificar"></a></div></td>
							<td width="5%"><div align="center"><a href="#"><img src="../img/ver.png" width="16" height="16" border="0" onClick="ver_albaran(<?php echo mysql_result($res_resultado,$contador,"entrada_reparacion.id")?>)" title="Visualizar"></a></div></td>
						</tr>
						<? $contador++;
							}
						?>			
					...
					<table class="fuente8" width="87%" cellspacing=0 cellpadding=3 border=0>
						<tr>
					<td width="100%" class="mensaje"><?php  echo "No hay ninguna orden de entrada por reparación que cumpla con los criterios de b&uacute;squeda";?></td>
					    </tr>
					</table>					
					<? } ?>					
				</div>
