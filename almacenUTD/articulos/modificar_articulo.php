				...
				<input id="codarticulo" name="codarticulo" value="<?php echo $codarticulo?>" type="hidden">
					...
					<?php
					  ...
					  ?>
                        ...
                        <tr>
							<td>2da. Ubicaci&oacute;n</td>
							<td colspan="2"><select id="cboUbicacion2" name="AcboUbicacion2" class="comboGrande">
							<option value="0">Todas las ubicaciones</option>
								<?php
								while ($contador2 < mysql_num_rows($res_ubicacion)) { 
									if ( mysql_result($res_ubicacion,$contador2,"codubicacion") == $codubicacion2) { ?>
								<option value="<?php echo mysql_result($res_ubicacion,$contador2,"codubicacion")?>" selected><?php echo mysql_result($res_ubicacion,$contador2,"nombre")?></option>
								<? } else { ?> 
								<option value="<?php echo mysql_result($res_ubicacion,$contador2,"codubicacion")?>"><?php echo mysql_result($res_ubicacion,$contador2,"nombre")?></option>
								<? }
								$contador2++;
								} ?>				
								</select>							</td>
					  ...
					  <tr>
						...
						<tr>
							<td>Fecha de alta</td>
							<td colspan="2"><input NAME="fecha" type="text" class="cajaPequena" id="fecha" size="10" maxlength="10" readonly value="<?php echo mysql_result($rs_query,0,"fecha_alta")?>"> <img src="../img/calendario.png" name="Image1" id="Image1" width="16" height="16" border="0" onMouseOver="this.style.cursor='pointer'">
        <script type="text/javascript">
					...
					button     : "Image1"
                        ...
                        <td>Fecha de Vencimiento</td>
							<td colspan="2"><input NAME="fechaven" type="text" class="cajaPequena" id="fechaven" size="10" maxlength="10" readonly value="<?php echo mysql_result($rs_query,0,"fecha_venci")?>"> <img src="../img/calendario.png" name="Image2" id="Image2" width="16" height="16" border="0" onMouseOver="this.style.cursor='pointer'">
        <script type="text/javascript">
					Calendar.setup(
					  {
					inputField : "fechaven",
					...
					button     : "Image2"
                        ...
                        <tr>
						 <td>Aviso Vencimiento</td>
						  <td colspan="2"><select name="aviso_venc" id="aviso_venc" class="comboPequeno">
						  <?php if (mysql_result($rs_query,0,"aviso_venci")==0) { ?>
						  <option value="0" selected="selected">No</option>
						  <option value="1">Si</option>
						  <? } else { ?>
						  <option value="0">No</option>
					  ...
					  <tr>
                       ...
                       <?php
					  	...
					  	$query_linea="SELECT li.idLinea, CONCAT(ma.marca,'-',li.name_linea)AS nom FROM linea li JOIN marca_vehiculo ma ON li.marca_vehiculo_id_marca=ma.id_marca ORDER BY nom ASC";
						$res_linea=mysql_query($query_linea);
						$contador=0;
					  ?>
						<tr>
							<td>Linea</td>
							<td colspan="2"><select id="cbolinea" name="AcLinea" class="comboGrande">
							<option value="0">Todas las Lineas</option>
								<?php
								while ($contador < mysql_num_rows($res_linea)) { 
									if ( mysql_result($res_linea,$contador,"li.idLinea") == $codlinea) { ?>
								<option value="<?php echo mysql_result($res_linea,$contador,"li.idLinea")?>" selected><?php echo mysql_result($res_linea,$contador,"nom")?></option>
								<? } else { ?> 
